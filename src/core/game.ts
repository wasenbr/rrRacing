...
export class Game {
  ...
  private sizeH = 0;
  private sizePr = 0;
  /** Fração do buffer da tela usada no desenho (resolução dinâmica abaixo da alocada; ver resize). */
  private vpK = 1;

  /** Viewport (e recorte) em pixels CSS dentro do canto usado do buffer. */
  private viewport(x: number, y: number, w: number, h: number, scissor = false): void {
    const k = this.vpK;
    this.renderer.setViewport(x * k, y * k, w * k, h * k);
    if (scissor) this.renderer.setScissor(x * k, y * k, w * k, h * k);
  }
  ...
  /** exact: recria o buffer no tamanho certo (preparo da largada, fora da vista), sem sobra do menu. */
  private resize(exact = false): void {
    ...
    this.redraw = true;
    // só recria o buffer da tela quando o tamanho muda ou a densidade sobe acima da alocada: recriar
    // custa 100–250 ms (TV 4K na integrada), um engasgo por degrau da resolução dinâmica. Descendo,
    ...
    } else if (!realloc && pr !== this.sizePr * this.vpK) this.perfEvent('resize', `${w}x${h}@${pr.toFixed(2)} canto ${this.phase}`);
    this.vpK = pr / this.sizePr;
    const canvas = this.renderer.domElement;
    canvas.style.transformOrigin = '0 100%';
    canvas.style.transform = this.vpK < 1 ? `scale(${1 / this.vpK})` : '';
    // retângulo da tela para as etiquetas dos rivais (lido aqui, uma vez, não por rival no desenho)
  ...
  private render(alpha: number, frameDt: number, simulating: boolean, ahead: number): void {
    ...
    if (this.rig.mode === 'cockpit' && this.phase !== 'menu') {
      ...
      this.viewport(m.x, h - m.y - m.h, m.w, m.h, true);
