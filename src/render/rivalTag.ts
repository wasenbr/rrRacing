...
export class RivalTag {
  ...
  constructor(
    ...
    this.sprite.scale.set(4.6, 2.01, 1);
    ...
    this.sprite.onBeforeRender = (renderer, _scene, camera) => {
      NDC.setFromMatrixPosition(this.sprite.matrixWorld).project(camera);
  ...
  update(armorRatio: number, place: number): void {
    const dots = 6;
    ...
    ctx.lineWidth = 5;
    ctx.strokeText(this.name, 128, 60);
    ctx.fillStyle = '#f0f0f0';
    ctx.fillText(this.name, 128, 60);
