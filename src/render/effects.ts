...
class ParticlePool {
  ...
  emit(p: Omit<Particle, 'max'>): void {
    ...
    this.items.push({ ...p, max: p.life, rot: Math.random() * Math.PI * 2, spin: (Math.random() - 0.5) * 1.6 });
  }

  update(dt: number): void {
    const alive: typeof this.items = [];
...
export class Effects {
  ...
  dust(x: number, y: number, z: number, color: number, amount = 1): void {
    this.smoke.emit({
      ...
      c0: new THREE.Color(color), c1: new THREE.Color(color).multiplyScalar(0.8), gravity: -0.3, drag: 2,
    });
  }

  /** Jato do nitro: chama azul saindo da traseira. */
  nitro(x: number, y: number, z: number, heading: number): void {
    const bx = -forwardX(heading);
    const bz = -forwardZ(heading);
    this.fire.emit({
      x: x + (Math.random() - 0.5) * 0.3, y: y + (Math.random() - 0.5) * 0.2, z: z + (Math.random() - 0.5) * 0.3,
      vx: bx * (8 + Math.random() * 6), vy: Math.random() * 1.5, vz: bz * (8 + Math.random() * 6),
      life: 0.18 + Math.random() * 0.12, size0: 0.7, size1: 0.1,
      c0: HOT(0x9ae0ff, 4), c1: HOT(0x2050ff, 1.5), gravity: 0, drag: 4,
    });
  }
  ...
  explosion(x: number, y: number, z: number, big = true): void {
    ...
    for (let i = 0; i < (big ? 10 : 4); i++) {
      this.fire.emit({
        x: x + (Math.random() - 0.5) * 1.2 * s, y: y + 1 + Math.random() * s, z: z + (Math.random() - 0.5) * 1.2 * s,
        vx: (Math.random() - 0.5) * 3, vy: 2 + Math.random() * 3, vz: (Math.random() - 0.5) * 3,
        life: 0.5 + Math.random() * 0.35, size0: 2.4 * s, size1: 3.4 * s,
        c0: HOT(0xffc060, 1.7), c1: HOT(0xc02800, 0.5), gravity: -2, drag: 2.5,
      });
    }
    const n = big ? 46 : 16;
    for (let i = 0; i < n; i++) {
    ...
    // destroços escuros voando em arco
    for (let i = 0; i < (big ? 14 : 4); i++) {
      const a = Math.random() * Math.PI * 2;
      const sp = 6 + Math.random() * 8;
      this.smoke.emit({
        x, y: y + 1, z,
        vx: Math.cos(a) * sp, vy: 6 + Math.random() * 7, vz: Math.sin(a) * sp,
        life: 0.8 + Math.random() * 0.5, size0: 0.35, size1: 0.3,
        c0: new THREE.Color(0x1a1a1a), c1: new THREE.Color(0x101010), gravity: 22, drag: 0.4,
      });
    }
    for (let i = 0; i < (big ? 34 : 10); i++) {
      const a = Math.random() * Math.PI * 2;
      this.smoke.emit({
        x: x + Math.cos(a), y: y + 0.8 + Math.random() * 1.5, z: z + Math.sin(a),
        vx: Math.cos(a) * 2.2, vy: 2 + Math.random() * 4.5, vz: Math.sin(a) * 2.2,
        life: 1.4 + Math.random() * 1.4, size0: 1.6 * s, size1: 6 * s,
        c0: new THREE.Color(0x3a2e26), c1: new THREE.Color(0x2a2a2a), gravity: -1, drag: 1.2,
      });
    }
    this.sparks(x, y + 0.8, z, big ? 36 : 12);
    ...
    flash.light.intensity = big ? 260 : 90;
    ...
    ring.scale.setScalar(big ? 1 : 0.5);
    this.group.add(ring);
  ...
  jumpJet(x: number, y: number, z: number): void {
    ...
    for (let i = 0; i < 12; i++) {
      ...
      this.smoke.emit({
        ...
        c0: new THREE.Color(0x9a948e), c1: new THREE.Color(0x3a3634), gravity: -0.2, drag: 2.5,
      });
    }
  }
  ...
  fall(x: number, y: number, z: number, style: string): void {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      (lava ? this.fire : this.smoke).emit({
        ...
        c0: lava ? HOT(splash, 3) : new THREE.Color(splash), c1: lava ? HOT(0xff2000, 1) : new THREE.Color(splash).multiplyScalar(0.8),
        gravity: 14, drag: 0.8,
      });
    }
  }
